...
// TailLast writes the last n lines of proxy.log to w.
...
// Follow streams new log lines until ctx is cancelled.
func Follow(ctx context.Context, w io.Writer, n int) error {
	...
	if err := TailLast(w, n); err != nil && !errors.Is(err, os.ErrNotExist) {
		// fine if the file doesn't exist yet; we'll wait for it
	}
	var f *os.File
	for {
		select {
